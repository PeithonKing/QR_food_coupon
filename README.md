...
1. **Clone the repository:**
   ...
   ```bash
   git clone https://github.com/PeithonKing/QR_food_coupon.git
   ```

2. **Ensure the correct Python version:**
   This project requires Python **>= 3.10.7** and has been tested to work with Python versions up to **3.12**. It hasn't been tested with conda, but it should work as expected. Make sure you have the correct version installed:
   ```bash
   python --version
   ```

3. **Set up a Python virtual environment:**
   It's recommended to use a virtual environment to manage project dependencies. Follow the appropriate instructions below based on your preferred environment:
   ...
   - **For Conda virtual environment:**
     ```bash
     conda create --name qr_coupon_env python=3.12
     conda activate qr_coupon_env
     ```

4. **Install required dependencies:**
   Once your virtual environment is set up, navigate to the project directory and install the necessary dependencies:
   ```bash
   pip install -r requirements.txt
   ```

## Step 2: Setting Up Google Drive and Google Cloud Project

1. Follow [this](https://developers.google.com/workspace/guides/create-project) documentation page to create a Google Cloud Project using a suitable google account.
2. Go to your [google cloud console](https://console.cloud.google.com) and select the project you just created. On the top left corner, click on the hamburger menu and select `APIs & Services`, and then `Credentials`.
3. If you receive a warning stating that you haven’t configured the `OAuth consent screen`, click on `Configure consent screen` and fill in the required details. **Make sure to select "Internal"** users, not "External".
   - If you don't see the option for "Internal", it's likely that you're using a non-NISER/institute email. In that case, you **must stop** and restart the process using your **institute email**. 
   - Choosing "External" would require additional setup and is prone to complications, so it’s best to stick with the "Internal" option.

   You can skip the `Scopes` section for now. After configuring, click the `Back to Dashboard` button to return.
4. Now on the `Credentials` page, click on `Create Credentials` and select `OAuth client ID`. The application type should be `Desktop app` and the name can be anything. Click on `Create`.
5. You will have a pop-up saying `OAuth client created`. Click on `Download JSON` and save the file as `credentials.json` in the same directory as `uploadandshare.py`. The JSON file will look something like this:
...
6. Go to [this](https://console.cloud.google.com/flows/enableapi?apiid=drive.googleapis.com) to **enable the `Google Drive API` for your project**. Make sure the correct project is selected in the top left corner.

## Step 3: The Files Required
...
## Step 5: Uploading the QR Codes to Google Drive and Sharing Them with Concerned People
...
## Step 6: Running the Server and Scanning the QR Codes
